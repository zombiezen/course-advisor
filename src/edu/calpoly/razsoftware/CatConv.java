...
public static void main(String[] args) throws IOException
{
    ...
    InputStream inputStream = CatConv.class.getResourceAsStream("Cat.txt");
    ...
    while(scanner.hasNextLine())
    {
        ...
        //IF the current line matches the pattern signifying a new entry THEN
        if(currentLine.matches("[A-Za-z]{3,4} [0-9]{3} [^(]* .*[(].*"))
        {
            ...
            //SET index to 2
            int tokenIndex=2;
            //SET spl to be an array of tokens provided by splitting the current
            ...
            try
            {
                ...
                //SET the units of the course to the value in the parenthases
                newCatConvCourse.units = Integer.valueOf(spl[tokenIndex].charAt(1)-'0');
            }
            catch(Exception E)
            {
                System.out.println(currentLine);
            }
            //IF the current line indicates the class has more listings THEN
            if(currentLine.contains("(Also listed as"))
            {
                //add the alternate listings to the major list
                String[] altListing = currentLine.split("\\(Also listed as", 2);
        ...
        else if(CatConvCourses.size()>0)
        {
            ...
            CatConvCourse course = CatConvCourses.remove(CatConvCourses.size()-1);
    ...
    {
        //IF the current course description starts with "(Also listed as" THEN
        ...
        if(currentCourse.description.contains("Prerequisite:"))
        {
            ...
            String[] tokens = currentCourse.description.split("Prerequisite:");
    ...
    public static Set<Set<CatConvCourse>> parseReq(String in)
    {
        ...
        for(String s : stage1)
        {
            ...
            //FOR every pattern that matches a class, create a new class and add
            //it to the list
